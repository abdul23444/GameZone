    ...
    bsproduct.forEach((element) => {
        if (numCard < 4) {
            !bestSellproduct ? null : bestSellproduct.innerHTML += `
                    <div class="product-card-outer" style="width:25%; padding:10px;">
                    <div class="product-card">
                    <p style="display:none" class="product_id">${element.product.product_id}</p>
                    <div class="product-image">
                    <span class="discount-badge">${element.product.price.discount}</span>
...
const renderCart = () => {
    ...
    cartProducts.forEach((element) => {
        !myCart ? null : myCart.innerHTML += `<div class="cart-item">
                ...
                <div id="product_price">$${element.product.price.current}</div>
                <div><input type="number" value="1" min="1" max="20" id="quantityinp"></div>
                <div id="subtotal">$${element.product.price.current}</div>
            </div>`
    });
}
renderCart()
document.querySelector(".cart") ? document.querySelector(".cart").addEventListener("change", (e) => {
    ...
    let product_price = card.querySelector("#product_price").innerHTML.slice(1)
    card.querySelector("#subtotal").innerHTML = `$${quantity * product_price}`
    totalItemsRate(e)
    totalItemsRate1()
}) : null
...
const totalItemsRate = (e) => {
    ...
    subtotalList.forEach((element) => {
        totalAmount.push(Number(element.innerHTML.slice(1)))
    })
